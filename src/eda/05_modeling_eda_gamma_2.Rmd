...
with at least 1 non-zero observation.
...
#' @param rate_label character (label for Y axis)
#' @return ggplot object
plot_models_ci <- function(data, model_df, disease, chsa, rate_label) {
  cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
  
  ggplot(aes(x = YEAR, y = STD_RATE_PER_1000_2), data = data) +
    geom_point() +
    geom_ribbon(
      aes(x = YEAR, y = STD_RATE_PER_1000_2, ymin = STD_LCL_95, ymax = STD_UCL_95),
      alpha = 0.2,
      fill = "lightblue"
    ) +
    geom_ribbon(
      aes(x = x, y = y_fitted, ymin = ci_95_ll, ymax = ci_95_ul), 
      alpha = 0.2, data = model_df
    ) +
    geom_line(aes(x = x, y = y_fitted, color = model), data = model_df, linetype = 2) +
    labs(
...
compare_models_rw1 <- function(data, chsa, rate_label) {
  ...
  for (d in diseases) {
    ...
    if (any(disease_data$STD_RATE_PER_1000 == 0) & any(disease_data$STD_RATE_PER_1000 != 0)) {
      tryCatch(
        {plot_df <- rbind(fit_inla_rw1(disease_data))
          p <- plot_models_ci(disease_data, plot_df, d, chsa, rate_label)
          print(p)
        },
        error = function(e) {
          print(paste("Error with", d, chsa))
...
### Vanderhoof Rural RW1

This time, we'll plot the RW1 model (grey) compared to the original 95% confidence intervals (blue).
...
These rw1 CIs ranges seem a bit more reasonable compared to the original 95% confidence intervals.

We can try moving forward with this approach:

- Change zero values to 0.0001

- Fit only the rw1 gamma model on data with zeros previously 

- Keep the rw1/rw2 gamma model comparison approach on data without zeros
