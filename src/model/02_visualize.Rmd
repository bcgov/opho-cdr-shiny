...
life_prev_df <- load_files("results/model/LifePrevalence")
...
orig_inc_rate_df <- load_files("General/Data_T_CHSA/IncidenceRate") |>
  wrangle_orig_df()
orig_life_prev_df <- load_files("General/Data_T_CHSA/LifePrevalence") |>
  wrangle_orig_df()
```
...
renderPlot({
  life_selected_data() |>
    ggplot(aes(x = year, y = y_fitted)) +
    geom_ribbon(data = orig_life_selected_data(),
      aes(x = YEAR, y = STD_RATE_PER_1000, ymin = STD_LCL_95, ymax = STD_UCL_95),
      alpha = 0.2,
      fill = "lightblue"
    ) +
    ...
    labs(
      x = "Year", y = "Age Standardized Incidence Rate (per 1000)",
      title = paste(input$life_disease, "in", input$life_region)
    ) +
    theme_grey(base_size = 14) +
    theme(panel.grid.minor = element_blank()) +
    scale_x_continuous(n.breaks = 20)
