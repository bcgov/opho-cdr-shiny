...
merge_df <- function(df, new_df_model) {
  df |>
    left_join(new_df_model,
              by = c("DISEASE", "HEALTH_BOUNDARIES", "YEAR" = "year")) |>
...
health_bounds <- function(input) {
  (if (input == "Health Authorities")
    return(c(sort(
      unique(filter(inc_rate_df, GEOGRAPHY == "HA")$HEALTH_BOUND_NAME)
    )))
   else
     return(c(sort(
...
hovertemplate_line <-
  paste0(
    '<b>Health Region</b>: %{fullData.name}',
...
region_tab_hovertemplate_line <-
  paste0(
    '<b>Disease</b>: %{fullData.name}',
    '<br><b>%{yaxis.title.text}</b>: %{y:.2f}',
    '<br><b>Year</b>: %{x}',
    '<extra></extra>'
  )
...
x_axis_bar_spec <- function(title, so = 0) {
  list(
    title = list(text = title, standoff = so),
...
x_axis_line_spec <- function(title, so = 10) {
  list(
    title = list(text = title, standoff = 10),
    gridcolor = "#d9dadb",
    showline = T,
    linewidth = 1,
    linecolor = 'black'
