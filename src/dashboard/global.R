...
disease_dict <-
  c(
    "ALZHEIMER_DEMENTIA" = "Alzheimer's and Other Types of Dementia",
...
join_chsa <-
  sort(unique(levels(
    as.factor(joinpoint_df$HEALTH_BOUNDARIES)
  )))
join_disease <-
  sort(unique(levels(as.factor(
    joinpoint_df$DISEASE
  ))))
...
set.seed(2001)
DISEASE_colors <-
  data.frame(DISEASE = sample(unique(inc_rate_df$DISEASE)))
DISEASE_colors$Colors <-
  colorRampPalette(HA_colours[, 2])(length(unique(inc_rate_df$DISEASE)))
