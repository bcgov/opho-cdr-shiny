...
There are circumstances when the response and explanatory variables do not 
maintain a linear relationship, as such when there are abrupt changes resulting from 
effect on the response. In case of describing effect of time upon disease rates,
epidemiologists are usually interested in the break-point location and
the relevant regression parameters. Polynomial regression, regression splines 
and non-parametric smoothing are some of the many conventional methods to deal 
with such non-linear relationship. But here, they are not of help, as the 
change-points are constant *a priori* (regression splines) or
often, changepoints are ignored (smoothing splines and polynomial regression).
Moreover, loss of interpretability is another concern, when regression parameters 
are obtained in regression splines or polynomial regression approach
(@rj1990generalized). Moreover, the `R` package, `segmented`
...
we acknowledge our limitations and provide recommendations on future endeavors. 
For the dashboard, the main improvement lies in the speed. 
The performance is acceptable for a single user per session, though varies greatly by computers. 
