...
An R Shiny document was selected to visualize the models due to the large number of models,
which made it challenging to display at an individual level within other formats, 
such as a PDF report.
...
This data product allows data scientists and epidemiologists on the team
...
into the dashboard using a toggle switch, as described in the Dashboard section below.

```{r temporal-shiny, echo=FALSE, fig.cap="Fitted temporal models displayed in an R Shiny document. The R Shiny document allows users to explore the 11 956 temporal models relative to the original observations and confidence intervals.", out.width = '100%'}
...
First, we build an Shiny dashboard that allows health professionals and non-experts alike to explore the distribution of diseases. It is fully interactive, user-friendly and visually appealing and consistent with the BC government's color scheme. We implement all the proposed features, such as the layout design and yearly change animation. We also add interactivity that enrich the user experience, such as the hover information, linked highlighting effect. We also use methods to improve the app's performance on first start and during plots rendering. 
...
Finally, we explored methods such as Join-point Regression, which
provides insights into trends of chronic disease rates through temporal
analysis. The model provides epidemiologists with information on abrurpt
rise and fall in trend, which aids in decision-making. But, there are
several limitations such as the model is sensitive to missing values,
and diseases with stationary rates. In such cases, the model is not
useful, because of not having sufficient checkpoints in the data.

While we attempted to perfect the deliverables based on constant partner feedback, due to the short time frame of this capstone project, we acknowledge that there are areas we can improve on. Here are the recommendations on future endeavors. 

For the dashboard, the main improvement lies in the processing speed. Currently, we have only tested the dashboard locally for a single user per session. The performance in terms of processing speed is acceptable, but varies greatly by individual laptops. If it is foreseeable that multiple users will be active simultaneously, asynchronous programming can be applied to increase scalability. For more details on all our attempts to speed up the Shiny dashboards, please refer to [this Github issue](https://github.com/bcgov/opho-cdr-shiny/issues/34) in the project repository. 
